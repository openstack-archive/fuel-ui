...
var ClusterPage = React.createClass({
  ...
  statics: {
    ...
    title() {
      return app.cluster ? app.cluster.get('name') : '';
    },
    getTabs() {
      return [
    ...
    fetchData({params}) {
      ...
      cluster.get('transactions').fetch =
        utils.fetchClusterProperties(cluserId, {task_names: 'deployment'});
      ...
      .then(() => {
        ...
        return Promise.all(promises)
          .then(() => ({cluster, tabOptions: params}));
      });
    }
  },
  getDefaultProps() {
  ...
  getInitialState() {
    ...
    _.each(tabs, (tabData) => {
      if (tabData.tab.checkSubroute) {
        _.extend(states, tabData.tab.checkSubroute(_.extend({}, this.props, {activeTab})));
      }
    });
    return states;
  },
