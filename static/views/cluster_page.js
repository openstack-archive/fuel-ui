    ...
    loadProps(params, cb) {
      ...
      .then(() => {
        ...
        if (cluster.get('status') !== 'new') {
          allFetched.push(cluster.get('deployedSettings').fetch().catch(() => true));
          allFetched.push(cluster.get('deployedNetworkConfiguration').fetch().catch(() => true));
        }

        Promise.all(allFetched)
          .then(
            () => {
