  ...
  renderFooter() {
    ...
    return ([
      ...
      <ProgressButton
        ...
        showProgress={this.state.actionInProgress}
      >
        {selectedNodesAmount ?
...
export var StopDeploymentDialog = React.createClass({
  ...
  stopDeployment() {
    this.setState({actionInProgress: true});
    var task = new models.Task();
    return task.save({}, {url: _.result(this.props.cluster, 'url') + '/stop_deployment', type: 'PUT'})
      .done(() => {
        this.close();
        dispatcher.trigger('deploymentTaskStarted');
      })
...
export var DiscardSettingsChangesDialog = React.createClass({
  ...
  discard() {
    return this.proceedWith(this.props.revertChanges);
  },
  save() {
    return this.proceedWith(this.props.applyChanges);
  },
  getMessage() {
