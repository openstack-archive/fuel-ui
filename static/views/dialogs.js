...
export var ShowNodeInfoDialog = React.createClass({
  ...
  fetchActionsVMsConfModel(VMsConfModel) {
    this.setState({
      actionInProgress: false,
      VMsConf: JSON.stringify(VMsConfModel.get('vms_conf'))
    });
  },
  fetchActionsNodeAttributesModel(nodeAttributesModel) {
  ...
  componentDidMount() {
    ...
    if (this.props.node.get('pending_addition') && this.props.node.hasRole('virt')) {
      ...
      this.setState({actionInProgress: true});
      VMsConfModel.fetch().then(
        () => this.fetchActionsVMsConfModel(VMsConfModel),
        () => this.fetchActionsVMsConfModel(VMsConfModel)
      );
    }
    ...
    this.setState({actionInProgress: true});
    nodeAttributesModel.fetch().then(
      () => this.fetchActionsNodeAttributesModel(nodeAttributesModel),
      () => this.fetchActionsNodeAttributesModel(nodeAttributesModel)
    );
  },
