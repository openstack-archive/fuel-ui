...
var DeploymentTimeline = React.createClass({
  render() {
    ...
    isRunning = true;
    var timeStart = _.min(history.map('time_start'));
    var timeEnd = isRunning ? _.now() : _.max(history.map('time_end'));
    ...
    var getPx = (start, end) => Math.floor((end - start) / 4);
...
var NodeTimeline = React.createClass({
  ...
  render() {
    var {label, tasks, timeStart, timeEnd, getPx, backgrounds} = this.props;
    return (
      <div className='node-timeline'>
        ...
        <div className='timeline'>
          {tasks.map((task, index) => {
            ...
            var left = getPx(timeStart, task.get('time_start'));
            var width = getPx(task.get('time_start'), task.get('time_end') || timeEnd);
            return <div
              key={taskName}
              className='node-task'
