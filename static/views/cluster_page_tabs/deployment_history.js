...
var DeploymentHistory = React.createClass({
      ...
      filters: [
        {
          ...
          values: [],
          options: (history) => history.map('task_name')
        }, {
          name: 'node_id',
          label: i18n(ns + 'filter_by_node_id'),
          values: [],
          options: (history) => history.map('node_id')
        }, {
          name: 'status',
  ...
  toggleFilter(filter, visible) {
    ...
    var isFilterOpen = openFilter === filter;
    visible = _.isBoolean(visible) ? visible : !isFilterOpen;
    console.log(filter, visible ? filter : isFilterOpen ? null : openFilter);
    this.setState({
      openFilter: visible ? filter : isFilterOpen ? null : openFilter
    });
  },
  changeFilter() {
    return;
  ...
  render() {
    ...
    return (
      <div className='deployment-history-table row'>
        <div className='deployment-history-toolbar col-xs-12'>
          {
              ...
              <div className='well clearfix'>
                ...
                {_.map(filters,
                  (filter) => <MultiSelectControl
                    ...
                    onChange={_.partial(this.changeFilter, filter)}
