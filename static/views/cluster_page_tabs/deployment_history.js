...
var DeploymentHistory = React.createClass({
  ...
  render() {
    ...
    var nodes = ['master'].concat(
      _.without(_.compact(_.uniq(history.map('node_id'))), 'master')
    );
    var taskNames = _.uniq(_.compact(history.map(
      (task) => !_.isNull(task.get('node_id')) && task.get('task_name')
    ))).sort();
              ...
              <div className='node-history-table-title'>
                ...
                :
                  i18n(
                    ...
                    {node: cluster.get('nodes').get(nodeId).get('name')}
                  )
                }
              </div>
              <Table
