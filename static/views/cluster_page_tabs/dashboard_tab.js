...
var ClusterActionButton = React.createClass({
  getInitialState() {
    return {
      selectedNodeIds: _.pluck(this.props.nodes, 'id')
    };
  },
  getDefaultProps() {
    return {
  ...
  showSelectNodesDialog() {
    ...
    showDialog(
      SelectNodesDialog,
      {
        ...
        proceed: (selectedNodeIds) => this.setState({selectedNodeIds}),
        roles: cluster.get('roles'),
        nodeNetworkGroups: cluster.get('nodeNetworkGroups')
      }
    );
  ...
  render() {
      ...
      return (
        <div className='btn-group'>
          <button
            ...
            disabled={disabled}
            onClick={() => showDialog(dialog, {nodeIds: selectedNodeIds})}
          >
    ...
    return (
      ...
      >
        ...
        {i18n(
          this.props.ns +
            (selectedNodeIds.length ? 'button_title_all_nodes' : 'button_title_no_nodes'),
