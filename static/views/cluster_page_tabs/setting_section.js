...
var SettingSection = React.createClass({
  ...
  renderTitle(options) {
    var {metadata, sectionName} = options;
    var label = sectionName === 'common' ?
      i18n('cluster_page.settings_tab.groups.common')
    :
      metadata.label || sectionName;
    return metadata.toggleable ?
      <Input
        {... _.pick(options, 'disabled', 'tooltipText', 'onChange')}
        type='checkbox'
        name='metadata'
        label={label}
      ...
      />
    :
      <span className={'subtab-group-' + sectionName}>{label}</span>;
  },
        ...
        <h3>
          {metadata.toggleable ?
            <Input
              ...
              tooltipText={showSettingGroupWarning && groupWarning || '12345'}
