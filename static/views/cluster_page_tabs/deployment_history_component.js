...
import {DEPLOYMENT_HISTORY_TASK_STATUSES, DEPLOYMENT_HISTORY_TASK_ATTRIBUTES} from 'consts';
...
var DeploymentHistory = React.createClass({
      ...
      filters: [
        {
          ...
          values: [],
          options: (history) => _.uniq(history.map('node_id')),
          addOptionsFilter: true
        }, {
          name: 'status',
          label: i18n(ns + 'filter_by_status'),
          values: [],
          options: () => DEPLOYMENT_HISTORY_TASK_STATUSES
        }
      ]
    };
  },
  ...
  render() {
    ...
    var filteredTasks = history.filter(
        ...
        <div className='row'>
          <div className='history-table col-xs-12'>
            {filteredTasks.length ?
              <Table
                head={
                  DEPLOYMENT_HISTORY_TASK_ATTRIBUTES
                    .map((attr) => ({label: i18n(ns + attr + '_header')}))
                    .concat([{label: ''}])
                }
                body={_.map(filteredTasks,
                  (task) => DEPLOYMENT_HISTORY_TASK_ATTRIBUTES
