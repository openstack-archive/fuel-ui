...
var EditNodeInterfacesScreen = React.createClass({
  ...
  updateBondOffloading(bond, slaves, updateStates = true) {
    var offloadingModes = slaves
      .map((ifc) => ifc.get('meta').offloading_modes || [])
    ...
    if (sectionName === 'offloading') {
      return (
        <OffloadingModesSubtab
          {..._.pick(this.props,
            'locked', 'interface', 'attributes', 'validate', 'offloadingModes'
          )}
  ...
  render() {
    ...
    if (_.size(attributes.get('offloading.modes.value'))) {
      return <OffloadingModes
        {... _.pick(this.props, 'interface', 'attributes', 'offloadingModes', 'locked')}
      />;
    }
