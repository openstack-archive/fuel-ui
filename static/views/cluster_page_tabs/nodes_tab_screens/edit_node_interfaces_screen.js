...
var EditNodeInterfacesScreen = React.createClass({
  ...
  getInitialState() {
    ...
    this.props.nodes.each((node, nodeIndex) => {
      ...
      _.each(justInterfaces, (ifc, index) => {
        ...
        interfacesByIndex[index] = _.union(interfacesByIndex[index], [nodeIndex ? ifc : firstNodeInterfaces[index]]);
      });
    });
    return {
  ...
  getAvailableBondingTypesForInterfaces(interfaces) {
    return _.intersection(... _.compact(_.map(
      interfaces, this.getAvailableBondingTypes
    )));
  },
  render() {
    ...
    var availableBondingTypes = interfaces.reduce(
      (result, ifc) => {
        ...
        result[ifc.get('name')] =
          this.getAvailableBondingTypesForInterfaces(interfacesSlice);
        return result;
      }, {}
    );
