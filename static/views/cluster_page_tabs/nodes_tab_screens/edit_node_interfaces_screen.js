...
var EditNodeInterfacesScreen = React.createClass({
  ...
  hasChangesInRemainingNodes() {
    ...
    //var initialInterfacesData = _.map(initialInterfacesData, this.interfacesPickFromJSON);
  ...
  updateWithLimitations(sourceInterface, targetInterface) {
    ...
    _.each(sourceAttributes.attributes, (section, sectionName) => {
      if (limitations[sectionName].equal) {
  ...
  validate() {
    ...
    interfaces.each((ifc) => {
      if (!_.includes(slaveInterfaceNames, ifc.get('name'))) {
        var interfaceErrors = {};
        var networkErrors = ifc.validate({networkingParameters, networks});
        if (!_.isNull(networkErrors)) interfaceErrors.networks = networkErrors;
        ifc.get('attributes').isValid({cluster, configModels, meta: ifc.get('meta')});
        if (!_.isNull(ifc.get('attributes').validationError)) {
    ...
    if (!_.isEqual(interfacesErrors, this.state.interfacesErrors)) {
  ...
  isSavingPossible() {
    return _.isEmpty(this.state.interfacesErrors) &&
  ...
  render() {
    ...
    var {
      ...
      interfacesErrors, initialInterfacesData
    } = this.state;
...
var NodeInterfaceAttributes = React.createClass({
  ...
  getSectionValueLabel(sectionName) {
    ...
    // each ifc attributes section has checkbox as its first control to enable
    // an appropriate feature on the particular interface
