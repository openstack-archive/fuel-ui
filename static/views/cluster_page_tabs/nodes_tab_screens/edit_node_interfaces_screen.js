    ...
    interfaces.each((ifc) => {
      if (!_.includes(slaveInterfaceNames, ifc.get('name'))) {
        var interfaceErrors = _.extend({},
            ...
            ifc.get('attributes').validate(_.extend({meta: ifc.get('meta')}, options))
        );
        ...
        {_.map(renderableSections, (sectionName) => {
          ...
          var settings = _.map(_.omit(section, 'metadata'),
            (setting, settingName) => {
              ...
              if (limitations[sectionName] && limitations[sectionName][result.settingName] &&
                  !limitations[sectionName][result.settingName].equal) {
                isRestricted = true;
              }
              return result;
            });
