...
var EditNodeInterfacesScreen = React.createClass({
  ...
  compareInterfaceAttributes(interfaces, sectionName) {
    // compare interface attributes have identical structure for all interfaces
    var firstIfcSection = interfaces[0].get('attributes').get(sectionName);
    return _.every(interfaces,
      (ifc) => ifc.get('attributes').get(sectionName) &&
        _.every(ifc.get('attributes').get(sectionName),
          (setting, settingName) => _.isEqual(
            _.omit(firstIfcSection[settingName], 'value'),
  ...
  getInterfacesLimitations(interfaces) {
    // section is editable when
    // 1) for all ifc meta.section.enabled==true or meta.section.enabled==undefined
    // 2) all ifc attributes differs only in value fields
    var limitations = {};
    ...
    _.each(firstIfcAttributes.attributes, (section, sectionName) => {
      var isEnabled = _.every(interfaces, (ifc) => {
        var metaName = sectionName === 'offloading' ? 'offloading_modes' : sectionName;
        var isEnabled = ifc.get(utils.makePath('meta', metaName, 'enabled'));
        return _.isUndefined(isEnabled) || isEnabled;
      });
      var isCompatible = false;
      if (sectionName === 'offloading') {
        var firstIfcMeta = firstIfc.get('meta');
        isCompatible = _.every(interfaces, (ifc) => {
          return _.isEqual(firstIfcMeta[sectionName], ifc.get(utils.makePath('meta', sectionName)));
        });
      } else {
        isCompatible = this.compareInterfaceAttributes(interfaces, sectionName);
      }
      limitations[sectionName] = {
        equal: isEnabled && isCompatible,
        shown: firstIfcAttributes.get(sectionName)
      };
    });
    return limitations;
  },
        ...
        meta: {
          dpdk: {
            available: _.every(interfaces,
                (ifc) => ifc.get('meta').dpdk && ifc.get('meta').dpdk.available
            )
          }
        },
        state: 'down'
...
var NodeInterfaceAttributes = React.createClass({
  ...
  getRenderableSections() {
    var attributes = this.props.interface.get('attributes');
    var meta = this.props.interface.get('meta') || {};
    return _.chain(_.keys(attributes.attributes))
      .filter((sectionName) => {
        if (attributes.get(utils.makePath(sectionName, 'type')) === 'hidden') return false;
        // some bond properties are not visible in tabbed interface
        var skipList = ['mode', 'lacp', 'lacp_rate', 'xmit_hash_policy', 'type'];
        if (_.includes(skipList, sectionName)) return false;
  ...
  getSectionDefaultSettingValue(sectionName) {
    // the value to be displayed in section title
    var section = this.props.interface.get('attributes').get(sectionName);
    var {settingName: name, value: value} =
      _.first(_.sortBy(_.reject(section, {type: 'hidden'}), 'weight'));
    if (sectionName === 'offloading') {
      value = this.makeOffloadingModesExcerpt();
    } else if (name === 'disabled') {
      // 'disabled' setting name is special case, bool logic inverted
      value = value ? i18n('common.disabled') : i18n('common.enabled');
    } else if (_.isNil(value) || value === '' || _.isNaN(value)) {
      value = i18n('common.default');
    } else if (_.isBoolean(value)) {
      value = value ? i18n('common.enabled') : i18n('common.disabled');
    } else if (_.isObject(value)) {
      // FIXME(jkirnosova): nned to think about this label
      value = 'custom_value';
    }
    return value;
  },
    ...
    return (
      <div className='properties-list'>
        {_.map(renderableSections, (sectionName) => {
          var {metadata = {}} = attributes.get(sectionName);
          ...
          var defaultValue = this.getSectionDefaultSettingValue(sectionName);
          return (
  ...
  checkRestrictions(action, setting) {
    return this.props.interface.get('attributes').checkRestrictions(
      {
        attributes: this.props.interface.get('attributes'),
  ...
  renderInterfaceSubtab() {
    ...
    if (sectionName === 'offloading') {
      return (
        <OffloadingModesSubtab
          {..._.pick(this.props, 'locked', 'interface', 'errors', 'validate')}
        />
      );
    }
    ...
    var settingsToDisplay = _.filter(_.keys(_.omit(section, 'metadata')), (settingName) => {
      ...
      var hideRestrictions = _.filter(restrictions, {action: 'hide'});
  ...
  render() {
    ...
    var defaultSubtab = _.find(renderableSections,
      (sectioName) => this.props.limitations[sectioName].equal
    );
    return (
