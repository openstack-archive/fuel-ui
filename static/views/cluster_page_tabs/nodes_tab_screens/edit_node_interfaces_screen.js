...
var NodeInterfaceAttributes = React.createClass({
  ...
  getRenderableSections() {
    ...
    var sortedAttributes = attributeNames.filter((sectionName) => {
      if (attributes.get(sectionName).type === 'hidden') {
        return false;
      }
      // some bond properties are not visible in tabbed interface
      var skipList = ['mode', 'lacp', 'lacp_rate', 'xmit_hash_policy', 'validationError'];
  ...
  renderInterfaceSubtab() {
    ...
    var settings = _.keys(_.omit(section, 'metadata', 'nic_plugin_id')).sort((key1, key2) => {
      var control1 = section[key1];
      var control2 = section[key2];
      return (control1.weight || 100) - (control2.weight || 100);
    });

    var sectionName = 'mtu' || activeSection;
    var settingsToDisplay = ['value'];
        ...
        {activeSection &&
          <SettingSection
            ...
            onChange={(settingName, value) => {
              attributes.set([activeSection, settingName, 'value'].join('.'), value);
            }}
            settings={attributes}
            locked={
              isLocked
            }
            checkRestrictions={() => { return {result: false, message: null};}}
          />
        }
        {
          _.map(settings, (settingName) => {
            var control = section[settingName];
            var props = {
              key: settingName,
              wrapperClassName: settingName + '-control',
              name: settingName,
              label: control.label,
              description: control.description || '',
              type: control.type,
              error: this.props.errors[activeSection + '_' + settingName],
              onChange: (name, value) => {
                attributes.set([activeSection, name, 'value'].join('.'), value);
              }
            };
            switch (control.type) {
              case 'text':
