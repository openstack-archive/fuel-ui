...
var EditNodeInterfacesScreen = React.createClass({
  ...
  updateWithLimitations(sourceInterface, targetInterface) {
    ...
    if(_.get(limitations, 'offloading_modes.equal', false)) {
      targetAttributes.offloading = _.cloneDeep(sourceAttributes.offloading);
    }

    _.each(sourceAttributes, (groupValue, groupName) => {
      if(groupName==='offloading') {
        return;
      }
      if(_.get(limitations, groupName + '.equal', false)) {
        targetAttributes[groupName] = _.cloneDeep(sourceAttributes[groupName]);
      }
    });
    targetInterface.set({
...
var NodeInterfaceAttributes = React.createClass({
  ...
  renderInterfaceSubtab() {
    ...
    var controls = _.keys(_.omit(group, 'metadata', 'nic_plugin_id')).sort((key1, key2) => {
      var control1 = group[key1];
      var control2 = group[key2];
      return control1.weight - control2.weight;
    });
