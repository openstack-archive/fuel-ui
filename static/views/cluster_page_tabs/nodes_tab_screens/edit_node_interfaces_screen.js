...
var EditNodeInterfacesScreen = React.createClass({
  ...
  componentWillMount() {
    this.setState({
      initialInterfacesData: this.interfacesToJSON(this.props.interfaces),
  ...
  compareInterfaceAttributes(interfaces, section, sectionName) {
    ...
    if (sectionName === 'offloading') {
      var omitStates = (modes) => _.map(modes, (mode) => _.omit(mode, 'state'));
      var firstIfcOffloadingModes = omitStates(interfaces[0].get('meta').offloading_modes);
      return _.every(interfaces,
        (ifc) => _.isEqual(firstIfcOffloadingModes, omitStates(ifc.get('meta').offloading_modes))
      );
    }
  ...
  hasChangesInRemainingNodes() {
    ...
    var {initialInterfacesData, limitations} = this.state;
    ...
    return _.some(nodes.slice(1), (node) => {
      ...
      return _.some(initialInterfacesData, (initialIfcData, index) => {
        ...
        return _.some(initialIfcData, (data, attribute) => {
          switch (attribute) {
            case 'attributes': {
              return !_.isEqual(... _.invokeMap(
                [data, interfacesData[index][attribute]],
                _.omit, omittedProperties // omit restricted parameters from the comparison
              ));
            }
            case 'slaves': {
  ...
  getIntersectedOffloadingModes(interfaces) {
    ...
    return offloadingModes.reduce(
      (result, modes) => this.findOffloadingModesIntersection(result, modes)
    );
  },
  ...
  bondInterfaces(bondType) {
    ...
    var bondName, offloadingModes, bondAttributes;
      ...
      _.each(bondAttributes.attributes, (section, sectionName) => {
        if (
          ...
          _.each(section, (setting, settingName) => {
            setting.value =
              firstSlaveAttributes.get(utils.makePath(sectionName, settingName, 'value'));
          });
        }
      });
    ...
    bondAttributes.set('offloading.modes.value', this.getOffloadingModeStates(offloadingModes));
...
var NodeInterfaceAttributes = React.createClass({
  ...
  makeOffloadingModesExcerpt() {
    ...
    // root modes only go to excerpt
    var offloadingModes = _.map(ifc.get('meta').offloading_modes || [], 'name');
    var offloadingModeStates = _.pick(attributes.get('offloading.modes.value'), offloadingModes);
  ...
  getSectionValueLabel(sectionName) {
    ...
    var name = _.chain(_.keys(attributes.get(sectionName)))
      .sortBy(
        (settingName) => attributes.get(utils.makePath(sectionName, settingName, 'weight'))
      )
      .find(
...
var OffloadingModesSubtab = React.createClass({
  ...
  render() {
    ...
    if (_.size(attributes.get('offloading.modes.value'))) {
