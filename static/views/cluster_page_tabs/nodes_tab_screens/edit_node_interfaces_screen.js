  ...
  updateWithLimitations(sourceInterface, targetInterface) {
    ...
    _.each(sourceAttributes.attributes, (section, sectionName) => {
      _.each(section, (setting, settingName) => {
        var settingLimitations = limitations[sectionName] && limitations[sectionName][settingName];
        var path = utils.makePath(sectionName, settingName);
        if (settingLimitations && settingLimitations.equal) {
...
var NodeInterface = React.createClass({
  ...
  getBondMode() {
    ...
    return ifc.get('mode') || (ifc.attributes.get('mode.value'));
  },
  ...
  render() {
    ...
    var hasPropertiesErrors = !_.isEmpty(attributes.validationError);
    ...
    if (activeSection === 'offloading') {
      ...
      return (
        <DPDKSubtab
          ...
          {..._.pick(this.props, 'nodesInterfaces', 'locked', 'getAvailableBondingTypes', 'errors')}
        />
      );
