...
var EditNodeInterfacesScreen = React.createClass({
  ...
  intersectOffloadingModes(interfaces) {
    var stripState = (modes) => {
      modes = _.map(modes, (mode) => {
        mode = _.omit(mode, 'state');
        mode.sub = stripState(mode.sub);
        return mode;
      });
      return modes;
    };
    var firstModes = _.first(interfaces).get('meta').offloading_modes;
    var isEqual = _.every(interfaces,
      (ifc) => {
        var modes = ifc.get('meta').offloading_modes;
        return _.isEqual(stripState(firstModes), stripState(modes));
      }
    );
    return {
      offloading_modes: isEqual ? firstModes : [],
      modes: _.first(interfaces).getAttributeValue('offloading.modes')
    };
  },
  bondInterfaces(bondType) {
    ...
    if (!bond) {
      ...
      var offloading = this.intersectOffloadingModes(interfaces);
      let bondProperties = {
        type: 'bond',
        name: bondName,
        mode: bondMode,
      ...
      bondProperties.attributes.offloading.modes.value = offloading.modes;
        ...
        >
          ...
          {!isCompact ? [
            <NodeInterfaceAttributes
              key='1'
              {... _.pick(this.props, 'interface', 'nodesInterfaces',
                'limitations', 'locked', 'bondingProperties')}
...
var NodeInterfaceAttributes = React.createClass({
  ...
  getRenderableSections() {
    ...
    var sortedAttributes = _.keys(attributes).filter((key) => {
      ...
      if (_.includes(['mode', 'lacp', 'lacp_rate', 'xmit_hash_policy'], key)) {
        return false;
      }
      ...
      var group1 = attributes[key1];
      var group2 = attributes[key2];
  ...
  changeBondType(newType) {
    this.props.interface.setAttributeValue('type__', newType);
  ...
  makeOffloadingModesExcerpt() {
    ...
    if (!ifcModes.length) {
      return states[!this.props.interface.getAttributeValue('offloading.disable')];
    }
    ...
    return (
      <div className='properties-list'>
        {_.map(renderableGroups, (key) => {
          var group = attributes[key];
          var metadata = group.metadata;
          var fields = _.map(_.omit(group, 'metadata'),
              (value, key) => {var val = _.clone(value); val.key = key; return val;});
          var groupClasses = {
            forbidden: false,
            'property-item-container': true,
            active: key === activeInterfaceSectionName && !this.state.collapsed,
            'text-danger': _.some(fields, (field) => this.props.errors[key + '_' + field.key])
          };
          var defaultField = _.first(_.sortBy(fields, 'weight'));
          ...
          return (
            <span key={metadata.label} className={utils.classNames(groupClasses, key)}>
    ...
    if (activeSection === 'offloading') {
      ...
      return (
        <OffloadingModesSubtab
          interface={ifc}
          offloadingModes={attributes.offloading}
          {..._.pick(this.props, 'locked', 'errors')}
        />
      );
      ...
      return (
        <DPDKSubtab
          ...
          {..._.pick(this.props, 'nodesInterfaces', 'locked', 'errors', 'getAvailableBondingTypes')}
        />
      );
      ...
      return (
        <SRIOVSubtab
          interface={ifc}
          {..._.pick(this.props, 'nodesInterfaces', 'locked', 'errors')}
        />
      );
    }
      ...
      <div className={utils.classNames('forms-box attributes', activeSection + '-section')}>
        {
          _.map(controls, (controlName) => {
            var control = section[controlName];
            var props = {
              key: controlName,
              wrapperClassName: controlName + '-control',
              name: controlName,
              label: control.label,
              ...
              onChange: (name, value) => {
                var path = ['attributes', activeSection, name, 'value'].join('.');
                ifc.set({[path]: value});
              }
            };
            switch (control.type) {
...
var DPDKSubtab = React.createClass({
  onChange(name, value) {
    this.props.interface.setAttributeValue(name, value);
  },
  render() {
