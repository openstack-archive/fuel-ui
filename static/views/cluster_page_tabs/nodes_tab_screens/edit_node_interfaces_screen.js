...
var EditNodeInterfacesScreen = React.createClass({
  ...
  componentWillMount() {
    this.setState({
      ...
      restrictions: this.getEditRestrictions()
    });
  },
  componentDidMount() {
    this.validate();
  },
  compareInterfacesProperties(ifcs, path,
                               iteratee = _.identity,
                               source = 'interface_properties') {
    // Checks if all the sub parameters are equal for all interfaces property
    var ifcProperties = _.map(ifcs.map((ifc) => {
      var ifcProperty = ifc.get(source);
      return _.get(ifcProperty, path, ifcProperty);
    }), iteratee);
    var uniqProperties = [_.first(ifcProperties)];
    _.reduce(ifcProperties, (previous, current) => {
      if (!_.isEqual(previous, current)) uniqProperties.push(current);
      return current;
    });

    return {
      equal: uniqProperties.length === 1,
      shown: _.first(uniqProperties)
    };
  },
  getEditRestrictions() {
    // Gets restrictions for interfaces parameters editing.
    ...
    return this.props.interfaces.reduce((result, ifc, index) => {
      var ifcs = this.props.nodes.map((node) => {
        return node.interfaces.at(index);
      });
      var key = ifc.isBond() ? ifc.get('name') : ifc.get('id');
      result[key] = {
        ...
        dpdk: this.compareInterfacesProperties(ifcs, 'dpdk.available'),
        sriov: this.compareInterfacesProperties(ifcs, 'sriov.available'),
  ...
  interfacesPickFromJSON(json) {
    ...
    return _.pick(json, [
      'id', 'name', 'assigned_networks', 'mode', 'type', 'slaves', 'bond_properties',
      ...
      return _.any(initialInterfacesData, (ifcData, index) => {
        return _.any(ifcData, (data, attribute) => {
          ...
          var restrictions = this.state.restrictions[
            ifcData.type === 'bond' ? ifcData.name : ifcData.id
          ];

          var omittedProperties = _.filter(
            _.keys(restrictions),
            (key) => !_.get(restrictions[key], 'equal', true)
          );

          switch (attribute) {
            case 'offloading_modes': {
              // Do not compare offloading modes if they differ
              if (!_.get(restrictions, 'offloading_modes.equal', false)) return false;
  ...
  updateWithRestrictions(updaterIfc, updateeIfc) {
    // Interface parameters should be updated with respect to restrictions:
    // restricted parameters should not be changed
    var restrictions = this.state.restrictions[updateeIfc.get('id')];
    var updateeIfcProperties = updateeIfc.get('interface_properties');
    var updaterIfcProperties = updaterIfc.get('interface_properties');

    if (updateeIfc.get('offloading_modes')
        && _.get(restrictions, 'offloading_modes.equal', false)) {
      updateeIfc.set({
        offloading_modes: updaterIfc.get('offloading_modes')
      });
      // If set of offloading modes supported is the same, disable_offloading
      // parameters updated as well (it is probably obsolete)
      var disableOffloading = _.get(updaterIfcProperties, 'disable_offloading');
      if (!_.isUndefined(disableOffloading)) {
        _.set(updateeIfcProperties, 'disable_offloading', disableOffloading);
      }
    }

    _.each(updaterIfcProperties, (propertyValue, propertyName) => {
      // Set all unrestricted parameters values
      if (!_.isPlainObject(propertyValue) && _.get(restrictions, propertyName + '.equal', false)) {
        _.set(updateeIfcProperties, propertyName, propertyValue);
      }
    });
    updateeIfc.set({
      interface_properties: updaterIfcProperties
    });
  },
  applyChanges() {
    ...
    return $.when(...nodes.map((node) => {
      ...
      node.interfaces.each((ifc, index) => {
        ...
        this.updateWithRestrictions(updatedIfc, ifc);
      });
  ...
  bondInterfaces(bondType) {
    ...
    if (!bond) {
      ...
      var bondName = this.props.interfaces.generateBondName(bondType === 'linux' ? 'bond' : 'ovs-bond');
  ...
  mergeRestrictions(restriction1, restriction2) {
    var r = _.merge(restriction1, restriction2, (value1, value2, ifcProperty) => {
      switch (ifcProperty) {
        case 'mtu':
        case 'offloading_modes':
        ...
        case 'dpdk':
          if (_.isUndefined(value1) || _.isUndefined(value2)) return;
  ...
  unbondInterfaces() {
    this.setState({actionInProgress: true});
    _.each(this.props.interfaces.where({checked: true}), (bond, index) => {
      ...
      <div className='row'>
        ...
        <div className='ifc-list col-xs-12'>
          {interfaces.map((ifc, index) => {
            ...
            var restrictions = this.state.restrictions[ifc.isBond() ? ifcName : ifc.get('id')];
...
var NodeInterface = React.createClass({
  ...
  renderConfigurableAttributes() {
    ...
    var offloadingRestricted = !restrictions.offloading_modes.equal;
      ...
      <div className='properties-list'>
        ...
        {_.map(ifcProperties, (propertyValue, propertyName) => {
          var {equal, shown} = _.get(
            restrictions, propertyName, {equal: true, shown: true}
          );
