...
var EditNodeInterfacesScreen = React.createClass({
  ...
  compareInterfaceAttributes(interfaces, sectionName) {
    ...
    var stripValues = (section) => {
      return _.omitBy(section, (value, key) => key === 'value');
    };
    var firstIfcAttributes = stripValues(
        interfaces[0].get(utils.makePath('attributes', sectionName)));
    return _.every(interfaces, (ifc) => {
      return _.isEqual(firstIfcAttributes,
          stripValues(ifc.get(utils.makePath('attributes', sectionName))));
    });
  },
  ...
  bondInterfaces(bondType) {
    ...
    _.each(bondLimitations, (sectionLimitations, sectionName) => {
      var limitations = bondLimitations[sectionName];
      if (limitations.equal && bondAttributes.getsectionName) {
        bondAttributes.set(utils.makePath(sectionName),
          limitations.shown);
      }
    });
...
var NodeInterfaceAttributes = React.createClass({
  ...
  getSectionDefaultSettingValue(sectionName, settings) {
    ...
    var {settingName: name, value: value} =
        _.first(_.sortBy(_.reject(settings, {type: 'hidden'}), 'weight'));
    if (sectionName === 'offloading') {
    ...
    return (
      <div className='properties-list'>
        {_.map(renderableSections, (sectionName) => {
          ...
          var isRestricted = limitations[sectionName] && limitations[sectionName].isEqual;
          var settings = _.map(_.omit(section, 'metadata'),
            (setting, settingName) => {
              return _.extend(_.clone(setting), {settingName});
            });
          var sectionClasses = {
          ...
          return (
            <span key={metadata.label} className={utils.classNames(sectionClasses, sectionName)}>
              ...
              {(metadata && metadata.label) || sectionName}:
              <button
