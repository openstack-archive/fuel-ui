...
var EditNodeInterfacesScreen = React.createClass({
  ...
  bondInterfaces() {
    ...
    if (!bond) {
      ...
      var isDPDKAvailable = !_.any(interfaces, (ifc) => _.isEmpty(ifc.getDPDKData()));
      if (isDPDKAvailable) {
        bondProperties.dpdk = {
          enabled: !_.any(interfaces, (ifc) => !ifc.getDPDKData().enabled),
          available: true
        };
      }
      ...
      bondProperties = _.cloneDeep(bond.get('interface_properties'));
      if (bond.getDPDKData().enabled) {
        bondProperties.dpdk.enabled = !_.any(interfaces, (ifc) => !ifc.getDPDKData().enabled);
      }
      bond.set({
