...
var EditNodeInterfacesScreen = React.createClass({
  ...
  getInitialState() {
    ...
    var viewMode = _.first(this.viewModes);
    ...
    return {
      ...
      indexByInterface,
      viewMode,
      bindAccordionEvents: viewMode === 'standard'
    };
  },
  componentWillMount() {
    this.setState({
  ...
  componentDidUpdate() {
    if (this.state.bindAccordionEvents) this.setState({bindAccordionEvents: false});
  },
  ...
  changeViewMode() {
    var newMode = _.first(_.without(this.viewModes, this.state.viewMode));
    this.setState({
      viewMode: newMode,
      bindAccordionEvents: newMode === 'standard'
    });
  },
  render() {
            ...
            if (!_.includes(slaveInterfaceNames, ifcName)) {
              return (
                <NodeInterfaceDropTarget
                  ...
                  viewMode={viewMode}
                  bindAccordionEvents={this.state.bindAccordionEvents}
                />
              );
            }
          })}
...
var NodeInterface = React.createClass({
  ...
  componentWillReceiveProps({viewMode}) {
  ...
  componentDidUpdate() {
    this.props.validate();
    if (this.props.bindAccordionEvents) {
          ...
          <div className={utils.classNames({
            ...
            {!!visibleErrors.length &&
              <div className='ifc-error alert alert-danger'>
                {_.map(visibleErrors, (error, index) => {
                  if (!visibleErrors[index + 1]) return error;
                  return [error, <br key={'br' + index}/>];
                })}
              </div>
            }
          </div>
