  ...
  updateWithLimitations(sourceInterface, targetInterface) {
    ...
    _.each(sourceAttributes.attributes, (section, sectionName) => {
      _.each(section, (setting, settingName) => {
        ...
        var settingLimitations = _.get(limitations, utils.makePath(path, 'equal'));
    ...
    interfaces.each((ifc) => {
      if (!_.includes(slaveInterfaceNames, ifc.get('name'))) {
        var interfaceErrors = _.extend({},
            ...
            ifc.get('attributes').validate({meta: ifc.get('meta')})
        );
...
var NodeInterfaceAttributes = React.createClass({
  ...
  getRenderableSections() {
    ...
    var sortedAttributes = attributeSections.filter((sectionName) => {
      ...
      if (!meta[sectionName] &&
        _.some(_.values(limitations[sectionName]), (field) => field.shown)) {
        return true;
      }
      ...
      return (weight1 || 100) - (weight2 || 100);
    });
  ...
  getSectionDefaultSettingValue(sectionName, settings) {
    ...
    var {settingName: name, value: value} = _.first(_.sortBy(settings, 'weight'));
    if (sectionName === 'offloading') {
      value = this.makeOffloadingModesExcerpt();
    } else if (name === 'disabled' || name === 'disable') {
      // API problem, offloading.disable should be offloading.disabled
        ...
        {_.map(renderableSections, (sectionName) => {
          ...
          return (
            <span key={metadata.label} className={utils.classNames(sectionClasses, sectionName)}>
              {sectionName === 'offloading' && isRestricted &&
  ...
  renderInterfaceSubtab() {
    ...
    if (activeSection === 'offloading') {
      ...
      return (
        <DPDKSubtab
          ...
          {..._.pick(this.props, 'nodesInterfaces', 'locked',
            'getAvailableBondingTypes', 'interface')}
        />
      );
      ...
      return (
        <SRIOVSubtab
          errors={errors}
          {..._.pick(this.props, 'nodesInterfaces', 'locked', 'interface')}
        />
      );
    }
...
var DPDKSubtab = React.createClass({
  onChange(name, value) {
    var {interface: ifc} = this.props;
    ifc.get('attributes').set(name, value);
    ifc.trigger('change', ifc);
  },
  render() {
    ...
    return (
      <div className='sriov-panel'>
        ...
        <Input
          ...
          error={errors && errors['sriov.enabled']}
        />
        {isSRIOVEnabled && [
          <Input
            key='sriov.numvfs'
            ...
            error={errors && errors['sriov.numvfs']}
          />,
          <Input
            key='sriov.physnet'
            type='text'
            ...
            error={errors && errors['sriov.physnet']}
