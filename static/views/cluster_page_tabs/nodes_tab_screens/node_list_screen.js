...
NodeListScreenContent = React.createClass({
    ...
    var normalizedFilters = _.map(activeFilters, (activeFilter) => {
      ...
      if (filter.values.length) {
        if (filter.isLabel) {
          ...
          filter.values = _.filter(filter.values, (value) => nodes.any({[filter.name]: value}));
        }
      }
      return filter;
    });
    if (!_.isEqual(_.pluck(normalizedFilters, 'values'), _.pluck(activeFilters, 'values'))) {
  ...
  processRoleLimits() {
    ...
    var clusterNodes = cluster.get('nodes').filter(
      (node) => !_.contains(selectedNodeIds, node.id)
    );
    ...
    _.each(processedRoleLimits, (roleLimit, roleName) => {
      if (_.contains(selectedRoles, roleName)) maxNumberOfNodes.push(roleLimit.limits.max);
    });
    return {
  ...
  hasChanges() {
    return this.props.showRolePanel && this.props.nodes.any((node) => {
  ...
  getFilterResults(filter, node) {
    ...
    switch (filter.name) {
      case 'roles':
        ...
        break;
      case 'status':
        result = _.contains(filter.values, node.getStatus()) ||
          _.contains(filter.values, 'pending_addition') && node.get('pending_addition') ||
          _.contains(filter.values, 'pending_deletion') && node.get('pending_deletion');
        break;
      case 'manufacturer':
      case 'cluster':
      case 'group_id':
    ...
    var filteredNodes = nodes.filter((node) => {
      ...
      if (
        ...
        _.every(node.pick('name', 'mac', 'ip'), (attribute) => {
          return !_.contains((attribute || '').toLowerCase(), search.toLowerCase());
        })
      ) {
        return false;
      }
      ...
      // filters
      return _.all(activeFilters, (filter) => {
...
ManagementPanel = React.createClass({
  ...
  isSavingPossible() {
    return !this.state.actionInProgress &&
      (this.props.selectedRoles.length || this.props.indeterminateRoles.length) &&
      this.hasChanges();
  },
  revertChanges() {
  ...
  applyChanges() {
    ...
    return cluster.assignNodes(nodes.models)
      .done(() => {
        $.when(
          ...
          <div key={group} className={group + ' row'}>
            ...
            <div className='col-xs-11'>
              {_.map(groupedRoles[group], (role) => {
                ...
                var roleName = role.get('name');
                var selected = _.contains(selectedRoles, roleName);
                return (
                  <Role
                    key={roleName}
                    ref={roleName}
...
NodeList = React.createClass({
  ...
  render() {
    ...
    var rolesWithLimitReached = _.keys(_.omit(processedRoleLimits,
