...
var HealthcheckTabContent = React.createClass({
  ...
  stopTests() {
    ...
    if (testruns.length) {
      ...
      return Backbone.sync('update', testruns).done(() => {
        this.setState({actionInProgress: false});
        this.startPolling(true);
      });
    }
        ...
        {!disabledState &&
          <div className='healthcheck-controls row well well-sm'>
            ...
            {(hasRunningTests || hasStoppingTests) ?
              (<ProgressButton className='btn btn-danger stop-tests-btn pull-right'
          ...
          <div key='testsets'>
            {testsets.map((testset) => {
              return <TestSet
                key={testset.id}
                testset={testset}
<<<<<<< 64a95d7e7fa66f0a0fc4109a7ab5d47210047c53
                testrun={this.props.testruns.find({testset: testset.id}) ||
=======
                testrun={testruns.findWhere({testset: testset.id}) ||
>>>>>>> Apply btn-progress class to buttons during actions in progress
