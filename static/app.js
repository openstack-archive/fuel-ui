...
class App {
  ...
  onRouteChange(prevState, nextState, replace, cb) {
    app.longFetch = _.some(nextState.routes, (route) => _.get(route.component, 'longFetch'));
  ...
  navigate(url, {replace} = {replace: false}) {
    this.routerComponent.router[replace ? 'replace' : 'push'](url);
  }
  ...
  logout() {
    ...
    _.defer(() => this.navigate('/login', {replace: true}));
  }
