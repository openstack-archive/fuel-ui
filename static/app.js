...
class App {
  ...
  onRouteChange(prevState, nextState, replace, cb) {
    console.log('---- Route from', prevState.location.pathname, nextState.location.pathname, '----');
    if (app.onLeave && prevState.location.pathname !== nextState.location.pathname) {
      $.when(_.result(app, 'onLeave'))
        .then(
          () => {
            console.log('Proceed with routing');
            this.onEnter(nextState, replace);
            cb();
          },
          () => {
            console.log('Abort routing:', cb);
            replace({
