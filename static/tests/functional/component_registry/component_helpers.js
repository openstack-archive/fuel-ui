...
_.defaults(Command.prototype, {
  ...
  clickIfExists(cssSelector) {
    return new this.constructor(this, function() {
      return this.parent
        .findAllByCssSelector(cssSelector).then(function (btn) {
          if (btn.length == 1) {
            btn[0].click();
          }
        }).end();
...
//        .findAllByCssSelector('a.dashboard.cluster-tab').then(function (btn) {
//          if (btn.length == 1) {
//            btn[0].click();
//          }
//        }).end()
//        .findAllByCssSelector('.btn-danger').then(function (btn) {
//          if (btn.length == 1) {
//            btn[0].click();
//            modal.waitToClose();
//          }
//        }).end()
//        .findAllByCssSelector('button.delete-environment-btn').then(function (btn) {
//          if (btn.length == 1) {
//            btn[0].click();
//            modal.waitToOpen();
//          }
//        }).end()
//        .findAllByCssSelector('button.remove-cluster-btn').then(function (btn) {
//          if (btn.length == 1) {
//            btn[0].click();
//            modal.waitToClose();
//          }
//        }).end();
  ...
  clickObjectByIndex(objectsCssSelector, index) {
    return new this.constructor(this, function() {
      return this.parent
        .findAllByCssSelector(objectsCssSelector).then(function (nodes) {
          if (index < 0) {
  ...
  bondInterfaces(itfIndex1, itfIndex2) {
    return new this.constructor(this, function() {
      _this = this
      return this.parent
        .clickObjectByIndex('.ifc-list > div .ifc-header .common-ifc-name .custom-tumbler',
                            itfIndex1)
        .clickObjectByIndex('.ifc-list > div .ifc-header .common-ifc-name .custom-tumbler',
                            itfIndex2)
