...
registerSuite(() => {
  ...
  return {
    ...
    'Interface bonding Modes'() {
      return this.remote
        .then(() => interfacesPage.bondInterfaces('eth1', 'eth2'))
        .then(() => interfacesPage.selectBondMode('bond0', 'active-backup'))
        .then(() => interfacesPage.selectBondMode('bond0', '802.3ad'))
        .then(() => interfacesPage.bondInterfaces('eth0', 'eth3'))
        .then(() => interfacesPage.selectBondMode('bond1', 'active-backup'))
        .then(() => interfacesPage.selectBondMode('bond0', 'balance-rr'));
    }
  };
});
