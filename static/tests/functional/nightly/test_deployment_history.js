...
function saveScreenshot(remote) {
  remote.takeScreenshot().then((buffer) => {
    var targetDir = process.env.ARTIFACTS || process.cwd();
    var filename = new Date().toTimeString();
    filename = targetDir + '/debug/' + filename + '.png';
...
registerSuite(() => {
  ...
  return {
    ...
    'Check "History" tab after third cluster deployment'() {
      ...
      return this.remote
        ...
        .then(() => genericLib.checkSelectorColors('Active and unhovered third "History point"',
                                           ...
                                           lgreenColor, dblueColor, lblueColor))
    ...
    'Check that "Table View" is worked'() {
      ...
      this.timeout = 1000 * 60 * 32;

//      var observedNodes = new Set();
      var observedNodeName;
...
//        .clickByCssSelector(filterByNode)
      ...
      for (let i = 2; i <= 8; i++) {
        chain = chain
          .findByCssSelector('table tr:nth-child(' + i + ') > td:nth-child(2)')
            .getVisibleText()
            .then((text) => observedNodeName = text)
            .end();
        if (observedNodeName === undefined) {
          chain = chain
            .findByCssSelector('table tr:nth-child(' + i + ') > td:nth-child(2) button:nth-child(1)')
              .getVisibleText()
              .then((text) => observedNodeName = text)
              .end();
          assert(observedNodeName === controllerName, 'Invalid controller node name. Expected "' +
                 observedNodeName + '" to be "' + controllerName + '"');
        } else {
          assert(observedNodeName === 'Master node', 'Invalid master node name. Expected "' +
                 observedNodeName + '" to be "Master node"');
        };
      };

//        .assertElementTextEquals(tableBodyRow + ':nth-child(2) td:nth-child(2)', 'Master node',
