      ...
      return this.remote
        ...
        .findByCssSelector('.current-time-marker')
          ...
          .then((value) => {
            ...
            timeMarkerPosition = value;
          })
          .end()
        // FIXME(kpimenova): Not stable element too. Sometimes there are no enabled buttons
        .assertElementAppears('.deployment-history-toolbar .zoom-controls .btn-group ' +
          'button:not(:disabled)', 20000, 'One of zoom control buttons is enabled')
        .then(
          pollUntil(
        ...
        // FIXME(kpimenova): That check should ferify that current time marker position is changed
        // during deployment. But this part is unstable - it's offten the same position
        .findByCssSelector('.current-time-marker')
          .getComputedStyle('left')
          .then((value) => {
            return assert.isTrue(parseInt(value, 10) > parseInt(timeMarkerPosition, 10),
        ...
        // Deployment History tab of finished deployment
        .then(() => clusterPage.goToTab('History'))
        .assertElementAppears('.deployment-history-table', 100,
