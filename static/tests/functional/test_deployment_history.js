...
registerSuite(() => {
  ...
  return {
    ...
    'Test deployment history tab of finished deployment'() {
      return this.remote
        .then(() => clusterPage.goToTab('History'))
        .assertElementAppears('.deployment-history-table', 200,
        ...
        .assertElementAppears('.filters', 200,
          'Filters for deployment history table are shown')
        .assertElementsExist('.filters .filter-by-task_name, .filters .filter-by-node_id,' +
          '.filters .filter-by-status', 3,
