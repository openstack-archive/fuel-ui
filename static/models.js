...
models.Node = BaseModel.extend({
  ...
  areInterfacesConfigurable() {
    var status = this.get('status');
    return status === 'discover' || status === 'error' || status === 'stopped';
  },
  ...
  .extend({
    ...
    mergePluginSettings(pluginNames) {
      ...
      var mergeSettings = (pluginName) => {
        ...
        this.attributes[pluginName] = _.extend(
          _.pick(plugin, 'metadata'),
        ...
        if (updateNetworkSettings === isNetworkGroup) {
          if (this.isPlugin(section)) {
            if (newSettings.get(sectionName)) {
              var pathToChosenVersionId = utils.makePath(sectionName, 'metadata', 'chosen_id');
              var pathToVersionsData = utils.makePath(sectionName, 'metadata', 'versions');
              this.set({
                [pathToChosenVersionId]: newSettings.get(pathToChosenVersionId),
                [pathToVersionsData]: newSettings.get(pathToVersionsData)
              });
              this.mergePluginSettings(section);
            }
          } else {
            _.each(section, (setting, settingName) => {
