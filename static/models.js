...
models.InterfaceAttributes = models.Settings.extend({
  ...
  validate(attrs, options = {}) {
    var configModels = {
      attributes: this,
      settings: options.cluster && options.cluster.get('settings')
    };
    var errors = this._super('validate', [attrs, _.extend({models: configModels}, options)]);
  ...
  validateSRIOV({meta}) {
    ...
    return this.get('sriov.enabled.value') &&
