...
models.Interface = Backbone.DeepModel
  ...
  .extend({
    ...
    validate(attrs, options) {
      ...
      if (this.get('interface_properties').sriov.enabled &&
        networks.length &&
    ...
    validateSRIOV({cluster}) {
      ...
      if (!sriov.enabled) return null;
      var ns = 'cluster_page.nodes_tab.configure_interfaces.validation.';
      var errors = {};
      if (cluster.get('settings').get('common.libvirt_type.value') === 'qemu') {
        errors.common = i18n(ns + 'sriov_qemu_alert');
      }
