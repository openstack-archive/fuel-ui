...
var fetchOptionsMixin = {
  ...
  fetch(options) {
    var fetchOptions = this.fetchOptions ?
      _.extend({}, {data: this.fetchOptions}, options) : options;
...
models.Cluster = BaseModel.extend({
  ...
  initialize(options) {
    ...
    }, (fetchOptions, subCollection) => this.get(subCollection).updateFetchOptions(
      _.extend({cluster_id: id}, fetchOptions))
    );
  },
  validate(attrs) {
    var errors = {};
